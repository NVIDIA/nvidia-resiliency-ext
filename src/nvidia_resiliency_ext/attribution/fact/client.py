...
class FactAttributionService:
    ...
    def create_failure_attributor(
        ...
        *,
        ...
        observation_nodes = sorted({_normalize_node_id(str(node)) for node in nodes})
    ...
    def submit_dmesg_text_observation(
        ...
        *,
        ...
        raw_streams, _ = dmesg_text_to_raw_loki_streams(
            dmesg_text,
            default_hostname=default_hostname,
            timestamp_start_ns=int(start_time.timestamp() * 1_000_000_000),
            prefilter=self.dmesg_prefilter,
        )
        if not raw_streams:
            return None
    ...
    def _build_attributor_info(
        ...
        *,
        ...
        cluster = (
            os.environ.get("NVRX_CLUSTER_NAME") or os.environ.get("SLURM_CLUSTER_NAME") or "unknown"
        )
        tenant = os.environ.get("SLURM_JOB_ACCOUNT") or os.environ.get("NVRX_TENANT") or "unknown"
        job_name = os.environ.get("SLURM_JOB_NAME") or role or "unknown"
        return {
            "workload": {
    ...
    @staticmethod
    def _write_result(path: str, result: FactAttributionResult) -> None:
