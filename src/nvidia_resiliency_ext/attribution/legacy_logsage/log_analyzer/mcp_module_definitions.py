"""
Source-checkout-only MCP registrations for legacy LogSage tools.

This module lives under ``attribution/log_analyzer`` so it is excluded from the
published wheel together with the LogSage/LangChain implementation.
"""

from typing import Any

from nvidia_resiliency_ext.attribution.legacy_logsage.combined_log_fr.combined_log_fr_mcp import (
...
def _log_analyzer_output_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "properties": {
            "module": {
...
def _combined_log_fr_input_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "properties": {
            "log_path": {
            ...
            "fr_path": {
                "type": "string",
                "description": (
                    "FR dump directory, a single dump file, or path prefix "
                    "(TORCH_FR_DUMP_TEMP_FILE style, e.g. /shared/_dump_ when _dump_0 "
                    "exists); path mode with log_path"
                ),
            },
            "input_data": {
                "type": "array",
...
def _combined_log_fr_output_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "properties": {
            "module": {
...
def register_legacy_logsage_mcp_modules() -> None:
    """Register source-checkout-only LogSage-backed MCP tools."""
    ...
    global_registry.register(
        ...
        output_schema=_log_analyzer_output_schema(),
        requires_llm=True,
        dependencies=[],
    )
    ...
    global_registry.register(
        ...
        input_schema=_combined_log_fr_input_schema(),
        output_schema=_combined_log_fr_output_schema(),
        requires_llm=True,
        dependencies=[],
    )
