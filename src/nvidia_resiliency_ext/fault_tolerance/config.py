...
class HealthLoggingConfig:
    ...
    def is_any_source_enabled(self) -> bool:
        return self.dmesg.enabled or self.healthcheck.enabled
...
class FaultToleranceConfig:
    ...
    * Dmesg evidence files (optional):
      - `health_logging.prefix` [str] absolute file prefix for per-node evidence files
      - `health_logging.dmesg.enabled` [bool] asks `nvrx-fact-agent` to write
        the collected dmesg text to a per-node file on failed cycles
