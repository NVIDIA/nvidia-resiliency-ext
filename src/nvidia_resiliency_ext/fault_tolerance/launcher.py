...
from nvidia_resiliency_ext.fault_tolerance.ft_attribution import LogAnalysisConfig
...
class LocalElasticAgent(SimpleElasticAgent):
    ...
    def _start_workers(self, worker_group: WorkerGroup) -> Dict[int, Any]:
        ...
        # Submit current cycle's log to attribution service (master node only, URL mode, before workers start)
...
def get_args_parser() -> ArgumentParser:
    ...
    # Attribution: --ft-attribution-loganalysis [lib|mcp|url]; default lib
    parser.add_argument(
        "--ft-attribution-loganalysis",
        "--ft_attribution_loganalysis",
        nargs="?",
        const="lib",
        default=None,
        dest="ft_attribution_loganalysis",
        metavar="MODE",
        help="Enable log analysis attribution on the Restart & progress state path. "
        "lib (default)= in-process; mcp= LogSage in MCP subprocess; "
        "url= HTTP service (e.g. http://127.0.0.1:8000).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Dataflow/Elasticsearch index for attribution posting (lib/mcp). Requires nvdataflow.",
    )
...
def config_from_args(args, launcher_pipe_read_fd=None, launcher_log_file=None) -> Tuple[LaunchConfig, Union[Callable, str], List[str]]:
    ...
    # Attribution: --ft-attribution-loganalysis [lib|mcp|url]; default lib
    attribution_loganalysis = getattr(fault_tol_cfg, "attribution_loganalysis", None)
    ...
    if attribution_loganalysis:
        timeout_sec = max(1, attribution_timeout)
        log_analysis_cfg = LogAnalysisConfig.from_ft_cli_value(
            attribution_loganalysis,
            timeout_seconds=timeout_sec,
            slack=ft_slack,
            dataflow_index=ft_dataflow_index,
        )
        rdzv_configs["log_analysis_config"] = log_analysis_cfg.to_dict()
