...
class NVLinkWindowHealthCheck(PynvmlMixin):
    ...
    Tier-1 first-probe edge case: the tier-1 support flag is latched on the
    first port probed (always port 0 of the first visible GPU). If port 0
    returns empty FIDs at process start -- e.g. an inactive port on an unusual
    topology -- tier-1 latches False process-wide and later active ports fall
    back to tier-2 / legacy. Restart after topology changes that bring tier-1
    ports online. On standard GB200 deployments port 0 is always active and
    ...
    def __init__(
        ...
        self._tier2_not_supported_warned = False
        # Warn only once when all APIs are unsupported for a port.
        self._api_not_supported_warned = False
        # Warn only once when a sample produced zero successful per-port reads
        # (every port returned None — likely a wedged NVML or all-unsupported
        # driver). Without this, the check silently passes forever.
        self._zero_ports_sampled_warned = False
    ...
    def _perform_health_check(self) -> bool:
        ...
        try:
            ...
            if not port_events and not self._zero_ports_sampled_warned:
                logger.warning(
                    "NVLinkWindowHealthCheck: zero ports returned data this sample. "
                    "Rolling windows will not advance and the check will pass-through "
                    "True until at least one port produces a successful read. Verify "
                    "NVML is responsive and NVLink is enabled on the visible GPUs."
                )
                self._zero_ports_sampled_warned = True
        except self.pynvml.NVMLError as e:
        ...
        for device_id, bad_ports in bad_ports_per_gpu.items():
            if len(bad_ports) >= self.gpu_ports_threshold:
                port_detail = ", ".join(f"port {p} ({f}/{w} flagged)" for p, f, w in bad_ports)
                logger.warning(
    ...
    def _read_via_field_values(self, handle, device_id: int, port: int) -> Optional[Dict[int, int]]:
        """
        ...
        if self._tier2_api_supported is not False and port < 6:
            ...
            if field_ids:
                try:
                    ...
                    if self._tier2_api_supported is None:
                        if not self._tier2_not_supported_warned:
                            logger.warning(
                                f"NVLinkWindowHealthCheck: nvmlDeviceGetFieldValues tier-2 not "
                                f"supported on device {device_id}: {e}. Falling back to legacy API."
                            )
                            self._tier2_not_supported_warned = True
                        self._tier2_api_supported = False

        return None

    def _read_via_legacy(self, handle, device_id: int, port: int) -> Optional[Dict[int, int]]:
        """
