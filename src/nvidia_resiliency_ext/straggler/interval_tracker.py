...
class ReportIntervalTracker:
    ...
    def _gather_report_interval(self):
        ...
        """
        assert (
            self.iter_interval is None
        ), "Report iteration interval has already been gathered."

        step_times = torch.tensor(self.step_times, dtype=torch.float32)
        median_step_time = torch.median(step_times)

        gathered_interval = (self.time_interval / median_step_time).to(
            torch.cuda.current_device()
        )
        if torch.distributed.is_initialized():
            torch.distributed.all_reduce(
                gathered_interval, op=torch.distributed.ReduceOp.MAX
            )
    ...
    def is_interval_elapsed(self):
        ...
        """
        return (self.iter_interval is not None) and (
            self.current_iter % self.iter_interval == 0
        )
