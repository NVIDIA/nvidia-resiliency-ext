...
def main():
    ...
    log_result = log_analyzer.run_sync(args)
    args.llm_analyze = False
    fr_result = CollectiveAnalyzer(args).run_sync(args)
    args.input_data = [log_result, fr_result]
    combined_result = CombinedLogFR(args).run_sync(args)
