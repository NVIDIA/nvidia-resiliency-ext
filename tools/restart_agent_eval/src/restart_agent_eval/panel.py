        ...
        behavioral_efficiency.append(
            {
                ...
                "exact_new_line_count": row.get("l1_kpi_tool_exact_new_line_count"),
                "exact_no_new_line_calls": row.get("l1_kpi_tool_exact_no_new_line_calls"),
                "semantic_new_evidence_calls": row.get("l1_kpi_tool_semantic_new_evidence_calls"),
                "semantic_no_new_evidence_calls": row.get(
                    "l1_kpi_tool_semantic_no_new_evidence_calls"
                ),
                "semantic_novelty_unassessed_calls": row.get(
                    "l1_kpi_tool_semantic_novelty_unassessed_calls"
                ),
                "final_context_impact": row.get("tool_final_context_impact"),
...
def _shared_l0_shape(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    first_view_sections = _dict(first_view_size.get("sections"))
    first_narrative = _dict(first_projection.get("narrative"))
    ...
    result.update(
        {
            ...
            "l0b_section_sizes": first_view_sections,
            "l0b_narrative_status": first_narrative.get("status"),
            "l0b_narrative_event_count": first_narrative.get("event_count"),
            "l0b_narrative_known_unknown_count": first_narrative.get("known_unknown_count"),
            "l0b_narrative_referenced_event_count": first_narrative.get("referenced_event_count"),
            "l0b_narrative_fanout_event_count": first_narrative.get("fanout_event_count"),
...
def _panel_row_l0_fields(state: _PanelRowState) -> dict[str, Any]:
    ...
    return {
        ...
        'l0_tool_calls_added_lines_beyond_initial_excerpt': state.l0_bundle_kpis.get(
            'tool_calls_added_lines_beyond_initial_excerpt'
        ),
...
def _panel_row_l1_fields(state: _PanelRowState) -> dict[str, Any]:
    return {
        ...
        'l1_kpi_tool_exact_new_line_count': state.l1_kpis.get('tool_exact_new_line_count'),
        'l1_kpi_tool_exact_no_new_line_calls': state.l1_kpis.get('tool_exact_no_new_line_calls'),
        'l1_kpi_duplicate_tool_calls': state.l1_kpis.get('duplicate_tool_calls'),
        'l1_kpi_tool_semantic_novelty_assessed_calls': state.l1_kpis.get(
            'tool_semantic_novelty_assessed_calls'
        ),
        'l1_kpi_tool_semantic_new_evidence_calls': state.l1_kpis.get(
            'tool_semantic_new_evidence_calls'
        ),
        'l1_kpi_tool_semantic_no_new_evidence_calls': state.l1_kpis.get(
            'tool_semantic_no_new_evidence_calls'
        ),
        'l1_kpi_tool_semantic_novelty_unassessed_calls': state.l1_kpis.get(
            'tool_semantic_novelty_unassessed_calls'
        ),
        'l1_kpi_tool_complete_source_scan_calls': state.l1_kpis.get(
            'tool_complete_source_scan_calls'
        ),
        'l1_kpi_tool_incomplete_source_scan_calls': state.l1_kpis.get(
            'tool_incomplete_source_scan_calls'
        ),
        'l1_kpi_tool_sample_truncated_calls': state.l1_kpis.get('tool_sample_truncated_calls'),
...
def _panel_row_operational_fields(state: _PanelRowState) -> dict[str, Any]:
    exact_novelty = _dict(state.tool_efficiency.get('exact_line_novelty'))
    semantic_novelty = _dict(state.tool_efficiency.get('semantic_evidence_novelty'))
    return {
        ...
        'complete_source_scan_calls': state.tool_efficiency.get('complete_source_scan_calls'),
        'incomplete_source_scan_calls': state.tool_efficiency.get('incomplete_source_scan_calls'),
        'sample_truncated_calls': state.tool_efficiency.get('sample_truncated_calls'),
        ...
        'tool_unique_new_line_count': exact_novelty.get('unique_new_line_count'),
    ...
    if row.get("context_efficiency") in {
        ...
        concerns.append(
            _concern(
                ...
                f"exact_no_new={row.get('tool_exact_no_new_line_calls')} "
                f"semantic_new={row.get('tool_semantic_new_evidence_calls')} "
                f"semantic_no_new={row.get('tool_semantic_no_new_evidence_calls')} "
                f"semantic_unassessed={row.get('tool_semantic_novelty_unassessed_calls')} "
                f"final_impact={row.get('tool_final_context_impact')}",
            )
        )
...
def _default_l0_kpis(
    *,
    ...
    exact_novelty = _dict(tool_efficiency.get("exact_line_novelty"))
    ...
    return {
        ...
        "tool_calls_added_lines_beyond_initial_excerpt": exact_novelty.get(
            "new_vs_initial_excerpt_count"
        ),
...
def _default_l1_kpis(
    *,
    ...
    exact_novelty = _dict(tool_efficiency.get("exact_line_novelty"))
    semantic_novelty = _dict(tool_efficiency.get("semantic_evidence_novelty"))
    return {
