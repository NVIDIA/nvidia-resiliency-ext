...
from .retry_policy_view import effective_allowed_retries, rule_expectation_correct
...
class CaseResult:
    ...
    deterministic_decision_correct: bool | None
    deterministic_policy_action_correct: bool | None
...
def score_case(
    *,
    ...
    enrichment_executed = target != "deterministic"
    l0_model_view = analyzer_trace.get("l0_model_view") or {}
    l0a_score = score_l0_against_gold(l0_bundle, case.label)
    l0b_score = (
        score_l0b_against_gold(l0_bundle, l0_model_view, case.label)
        if enrichment_executed
        else None
    )
    l1_evidence = verdict.get("l1_assessment") or (
        (analyzer_trace.get("l1") or {}).get("semantic_payload") or {}
    )
    l1_assessment = (
    ...
    if not isinstance(l1_assessment, dict):
        l1_assessment = (
            (verdict.get("l1_assessment") or {}).get("model_recovery_assessment")
            or verdict.get("model_recovery_assessment")
            or {}
        )
    if enrichment_executed:
        ...
        recovery_fields, recovery_correct = {}, None
    ...
    if not isinstance(decision_candidates, dict):
        decision_candidates = {}
    deterministic_verdict = decision_candidate_result(decision_candidates.get("deterministic"))
    ...
    if not deterministic_verdict and (
        target == "deterministic" or candidate_kind == "deterministic"
    ):
        deterministic_verdict = verdict
    if not enriched_verdict and candidate_kind == "l1_enriched":
        enriched_verdict = verdict
    deterministic_score = (
        score_semantic_view(deterministic_verdict, case.label, include_action=True)
    ...
    return CaseResult(
        ...
        l2_audit_correct=(
            _l2_audit_correct(analyzer_trace.get("l2_audit") or {}, case.label)
            if enrichment_executed
            else None
        ),
        ...
        deterministic_decision=(
            str(deterministic_verdict.get("decision") or "") or None
            if deterministic_verdict
            else None
        ),
        deterministic_decision_correct=(
            deterministic_score.get("action_correct") if deterministic_score is not None else None
        ),
        deterministic_policy_action_correct=(
            deterministic_score.get("policy_action_pass")
...
def _score_retry_policy(
    ...
    accepted_effective_rules = _accepted_values(expectation.get("accepted_effective_rules"))
    rule_correct = rule_expectation_correct(
        retry_policy,
    ...
    allowed_correct = (
        effective_allowed_retries(retry_policy) == _int_or_none(expected_allowed)
...
def _evidence_line_hit(verdict: dict[str, Any], label: dict[str, Any]) -> bool | None:
    ...
    if not expected:
        return None
    l2_grounding = verdict.get("l2_grounding") or {}
    evidence = l2_grounding.get("grounded_evidence") or verdict.get("evidence") or []
    actual = {
        int(item["line"])
    ...
    if isinstance(primary, dict) and primary.get("line") is not None:
        actual.add(int(primary["line"]))
...
def _unavailable_result(run_id: str, target: str, case: Case) -> CaseResult:
    return CaseResult(
        ...
        deterministic_decision_correct=None,
        deterministic_policy_action_correct=None,
...
def aggregate_results(run_id: str, target: str, results: list[CaseResult]) -> dict[str, Any]:
    ...
    return {
        ...
        "deterministic_decision_accuracy": _rate_defined(
            result.deterministic_decision_correct for result in scored
        ),
        "deterministic_policy_action_accuracy": _rate_defined(
            result.deterministic_policy_action_correct for result in scored
        ),
...
def _print_report(results: list[CaseResult], aggregate: dict[str, Any]) -> None:
    if aggregate.get("target") == "deterministic":
        _print_deterministic_report(results, aggregate)
        return
    print(
        "case | status | L1 recovery | L4 rule | retry count | exhaustion | deterministic | enriched | L1 effect | decision | evidence | primary"
    )
    print("--- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | ---")
    for result in results:
        print(
            ...
            f"{_mark(result.deterministic_decision_correct)} | "
    ...
    print(
        ...
        f"deterministic_decision_accuracy={aggregate['deterministic_decision_accuracy']} "
...
def _print_deterministic_report(
    results: list[CaseResult],
    aggregate: Mapping[str, Any],
) -> None:
    print(
        "case | status | L0A quality | primary | evidence | L4 rule | "
        "retry count | exhaustion | decision"
    )
    print("--- | --- | --- | --- | --- | --- | --- | --- | ---")
    for result in results:
        print(
            f"{result.case_id} | {result.status} | "
            f"{_mark(result.l0a_quality_correct)} | "
            f"{_mark(result.primary_anchor_hit)} | "
            f"{_mark(result.evidence_line_hit)} | "
            f"{_mark(result.l4_retry_rule_correct)} | "
            f"{_mark(result.l4_allowed_retries_correct)} | "
            f"{_mark(result.l4_exhaustion_correct)} | "
            f"{_mark(result.decision_correct)}"
        )
        if result.error:
            print(f"  error: {result.error}")
    print()
    print(
        f"scored={aggregate['scored_cases']} unavailable={aggregate['unavailable_cases']} "
        f"errors={aggregate['analyzer_errors']} "
        f"l0a_quality_accuracy={aggregate['l0a_quality_accuracy']} "
        f"primary_anchor_hit_rate={aggregate['primary_anchor_hit_rate']} "
        f"evidence_line_hit_rate={aggregate['evidence_line_hit_rate']} "
        f"l4_retry_rule_accuracy={aggregate['l4_retry_rule_accuracy']} "
        f"l4_allowed_retries_accuracy={aggregate['l4_allowed_retries_accuracy']} "
        f"l4_exhaustion_accuracy={aggregate['l4_exhaustion_accuracy']} "
        f"decision_accuracy={aggregate['decision_accuracy']}"
