...
def _run_review(
    ...
    *,
    ...
    for target in targets:
        ...
        if summary is None:
            if completed.returncode != 0 and not _review_artifacts_ready(
                paths,
                artifact_store=artifact_store,
            ):
                _print_unreviewable_target_failure(target, completed, paths)
                continue
...
def _review_artifacts_ready(
    paths: Mapping[str, Path],
    *,
    artifact_store: ArtifactStore,
) -> bool:
    """Return whether failed product output is complete enough for review."""

    try:
        ReviewContext.read(paths, artifact_store=artifact_store)
    except (KeyError, OSError, TypeError, ValueError, json.JSONDecodeError):
        return False
    return True


def _print_unreviewable_target_failure(
    target: RunTarget,
    completed: ProcessResult,
    paths: Mapping[str, Path],
) -> None:
    """Preserve the product failure instead of masking it with review errors."""

    print(
        f"{target.name}: product invocation failed with exit {completed.returncode}; "
        "review artifacts were not generated",
        file=sys.stderr,
    )
    detail = completed.stderr.strip() or completed.stdout.strip()
    if detail:
        print(detail, file=sys.stderr)
    stderr_path = paths.get("stderr_log")
    if stderr_path is not None:
        print(f"product stderr: {stderr_path}", file=sys.stderr)
    ...
    *,
    ...
    command = [
        *product_cli_command(args.python, log_path),
        ...
        str(run_dir / "deterministic_recommendation.json"),
...
def format_live_event(event: dict[str, Any]) -> str:
    ...
    if event_name == "deterministic_recommendation_ready":
        return (
            "live: deterministic recommendation ready " f"decision={event.get('decision')}{suffix}"
        )
...
def _build_review_summary(
    *,
    ...
    l0_selected_observation = context.l0_selected_observation
    l1_semantic_primary = context.l1_primary
    l1_observed_failures = context.l1_observed_failures
    l1_selected_observation = context.l1_selected_observation
    l2_grounded_primary = context.l2_primary
    l2_observed_failures = context.l2_observed_failures
    l2_selected_observation = context.l2_selected_observation
    ...
    deterministic_candidate = context.deterministic_candidate
    ...
    summary = {
        ...
        "observed_failures": [_observation_stage_view(item) for item in observed_failures],
        "selected_observed_failure": _observation_stage_view(selected_observation),
        "decision_evidence_sha256": _canonical_payload_sha256(decision_evidence),
        ...
        "observation_selection_by_stage": {
            "l0_deterministic": _observation_stage_view(l0_selected_observation),
            "l1_semantic": {
                **_observation_stage_view(l1_selected_observation),
                "observed_failure_count": len(l1_observed_failures),
            },
            "l2_grounded": {
                **_observation_stage_view(l2_selected_observation),
                "observed_failure_count": len(l2_observed_failures),
            },
            "l1_relation_to_l0": _primary_selection_relation(
                l0_selected_observation,
                l1_selected_observation,
                l0_bundle,
            ),
            "l2_relation_to_l0": _primary_selection_relation(
                l0_selected_observation,
                l2_selected_observation,
                l0_bundle,
            ),
        },
        "semantic_comparison": _semantic_comparison(l1_model_output),
        "history_identity_comparison": _history_identity_comparison(current_failure_facts),
        "failure_tracks": context.failure_tracks,
        "l3_history": context.cycle_history,
        "l4_path_selection": context.l4_path_selection,
        "decision_paths": {
            "deterministic": _decision_path_summary(
    ...
    summary["gold_score"] = score_against_gold(
        ...
        l1_evidence=l1.get("semantic_payload"),
        l2_grounding=context.l2_grounding,
        l2_audit=l2_audit,
...
def _build_review_metrics(
    *,
    ...
    selection_signals = build_model_selection_signals(
        ...
        semantic_safety=semantic_safety,
        route_execution_status=context.route_execution_status,
        route_l1_execution_assessment=context.route_l1_execution_assessment,
    )
...
def _decision_path_summary(
    ...
    return {
        ...
        "general_root_ceiling": general_root_ceiling(retry_policy),
        "selected_policy_ledger": selected_policy_ledger(retry_policy) or None,
...
def _observation_stage_view(observation: Mapping[str, Any]) -> dict[str, Any]:
    identity = observation.get("failure_identity")
    identity = identity if isinstance(identity, Mapping) else {}
    return {
        "id": observation.get("id"),
        "failure_class": observation.get("failure_class") or identity.get("mechanism"),
        "line": _int_or_none(observation.get("line")),
        "fault_outcome": observation.get("fault_outcome"),
        "causal_role": observation.get("causal_role"),
        "observation_fingerprint": observation.get("observation_fingerprint"),
        "observation_fingerprint_source": observation.get("observation_fingerprint_source"),
    }


def _semantic_comparison(l1_model_output: Mapping[str, Any]) -> dict[str, Any]:
    """Return only L1 scalar claims needed for cross-route comparison."""

    root_cause = l1_model_output.get("root_cause_assessment") or {}
    recovery = l1_model_output.get("model_recovery_assessment") or {}
    ...
    return {
        ...
        "failure_domain": _claim_comparison(recovery.get("failure_domain")),
        "retry_outlook_without_workload_change": _claim_comparison(
            recovery.get("retry_outlook_without_workload_change")
        ),
    }


def _claim_comparison(value: Any) -> dict[str, Any]:
    claim = value if isinstance(value, Mapping) else {}
    return {
...
def _history_identity_comparison(current_failure_facts: Mapping[str, Any]) -> dict[str, Any]:
    """Return the compact L3 identity inputs needed by panel comparisons."""

    affected_entity = current_failure_facts.get("affected_entity")
    affected_entity = affected_entity if isinstance(affected_entity, Mapping) else {}
    return {
        "source": current_failure_facts.get("source"),
        "identity_kind": current_failure_facts.get("identity_kind"),
        "history_identity_ready": current_failure_facts.get("history_identity_ready"),
        "root_fingerprint": current_failure_facts.get("root_fingerprint"),
        "observation_fingerprint": current_failure_facts.get("observation_fingerprint"),
        "observation_fingerprint_source": current_failure_facts.get(
            "observation_fingerprint_source"
        ),
        "selected_observation_line": current_failure_facts.get("selected_observation_line"),
        "selected_observation_causal_role": current_failure_facts.get(
            "selected_observation_causal_role"
        ),
        "affected_entity": {
            "kind": affected_entity.get("kind"),
            "fingerprint": affected_entity.get("fingerprint"),
        },
    }
...
def _review_index_markdown(
    *,
    ...
    lines = [
        ...
        "",
        "The per-model `result.json` keeps `l1_assessment` and `l2_grounding` separate, "
        "then publishes the final L4 result at the top level. The per-model `trace.json` "
        "is the deep diagnostic record. The exact L1 assessment is reproduced at the top "
        "of `review.md`.",
        "",
        "## Run Identity",
        "",
...
def _print_run_summary(summary: dict[str, Any], paths: dict[str, Path]) -> None:
    ...
    print(
        "decision={decision} primary={failure_class}@{line} "
        ...
        "redaction={redaction}".format(
            ...
            failure_class=summary["primary_failure"].get("failure_class"),
