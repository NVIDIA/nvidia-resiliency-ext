...
def _l1_kpis_markdown(l1_kpis: dict[str, Any]) -> list[str]:
    ...
    lines = [
        ...
        (
            ...
            f"exact_new_lines=`{l1_kpis.get('tool_exact_new_line_count')}`, "
            f"exact_no_new_line_calls=`{l1_kpis.get('tool_exact_no_new_line_calls')}`, "
            f"semantic_new_evidence_calls=`{l1_kpis.get('tool_semantic_new_evidence_calls')}`, "
            f"semantic_no_new_evidence_calls=`{l1_kpis.get('tool_semantic_no_new_evidence_calls')}`, "
            f"semantic_unassessed_calls=`{l1_kpis.get('tool_semantic_novelty_unassessed_calls')}`, "
...
def _l0_bundle_kpis_markdown(l0_bundle_kpis: dict[str, Any]) -> list[str]:
    ...
    section_sizes = view_size.get("sections") or {}
    narrative = projection.get("narrative") or {}
    ...
    lines = [
        ...
        (
            ...
            f"events=`{narrative.get('event_count')}`, "
            f"referenced_events=`{narrative.get('referenced_event_count')}`, "
            f"known_unknowns=`{narrative.get('known_unknown_count')}`"
        ),
        f"- section_sizes: `{section_sizes}`",
        ...
        (
            ...
            "calls_adding_lines_beyond_initial_excerpt="
            f"`{l0_bundle_kpis.get('tool_calls_added_lines_beyond_initial_excerpt')}`, "
            "calls_without_lines_beyond_initial_excerpt="
            f"`{l0_bundle_kpis.get('tool_calls_without_lines_beyond_initial_excerpt')}`, "
            f"semantic_new_evidence_calls=`{l0_bundle_kpis.get('tool_semantic_new_evidence_calls')}`, "
            f"semantic_no_new_evidence_calls=`{l0_bundle_kpis.get('tool_semantic_no_new_evidence_calls')}`, "
            f"semantic_unassessed_calls=`{l0_bundle_kpis.get('tool_semantic_novelty_unassessed_calls')}`, "
            f"duplicate_tool_calls=`{l0_bundle_kpis.get('duplicate_tool_calls')}`, "
            f"final_context_impact=`{l0_bundle_kpis.get('final_context_impact')}`"
        ),
        (
...
def _tool_efficiency_markdown(tool_efficiency: dict[str, Any]) -> list[str]:
    ...
    lines = [
        ...
        (
            ...
            "new_vs_initial_excerpt="
            f"`{exact_novelty.get('new_vs_initial_excerpt_count')}`, "
            "unique_new_lines="
            f"`{exact_novelty.get('unique_new_line_count')}`, "
            "already_in_initial_excerpt="
            f"`{exact_novelty.get('already_in_initial_excerpt_count')}`, "
            f"calls_without_new_lines=`{exact_novelty.get('calls_without_new_lines')}`"
        ),
        (
            "- semantic_evidence_novelty: "
            f"assessed_calls=`{semantic_novelty.get('assessed_calls')}`, "
            f"new_evidence_calls=`{semantic_novelty.get('new_evidence_calls')}`, "
            f"no_new_evidence_calls=`{semantic_novelty.get('no_new_evidence_calls')}`, "
            f"unassessed_calls=`{semantic_novelty.get('unassessed_calls')}`"
        ),
        (
            "- execution_flags: "
        ...
        (
            "- grep_scan: "
            f"complete_source_scan_calls=`{tool_efficiency.get('complete_source_scan_calls')}`, "
            f"incomplete_source_scan_calls=`{tool_efficiency.get('incomplete_source_scan_calls')}`, "
            f"sample_truncated_calls=`{tool_efficiency.get('sample_truncated_calls')}`"
        ),
        (
    ...
    if per_call:
        lines.extend(
            [
                "",
                "| turn | tool | args | result lines | exact new lines | exact repeated lines | full scan | semantic novelty | samples truncated | latency ms | flags |",
                "|---:|---|---|---:|---:|---:|---|---|---|---:|---|",
            ]
        )
        for item in per_call:
            ...
            call_exact_novelty = item.get("exact_line_novelty") or {}
            lines.append(
                "| "
                ...
                f"{call_exact_novelty.get('new_vs_initial_excerpt_count')} | "
                f"{call_exact_novelty.get('already_in_initial_excerpt_count')} | "
                f"{item.get('scan_complete')} | "
                f"{item.get('semantic_evidence_novelty')} | "
                f"{item.get('samples_truncated')} | "
