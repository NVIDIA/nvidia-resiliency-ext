...
from .retry_policy_view import general_root_ceiling, ledger_ratio, selected_rule_budget
...
def write_review_markdown(path: Path, summary: dict[str, Any]) -> None:
    ...
    selected_budget = selected_rule_budget(retry_policy)
    applied_capability = retry_policy.get("applied_recovery_capability") or {}
    ...
    lines = [
        ...
        (
            ...
            f"exhausted_by=`{retry_policy.get('exhausted_by') or []}`, "
            f"affected_entity=`{_entity_label(retry_policy.get('current_affected_entity'))}`, "
            f"applied_capability=`{applied_capability.get('capability_id')}`"
        ),
        (
            "- retry_ledgers: "
            f"general_root=`{ledger_ratio(general_ceiling)}` "
            f"exhausted=`{general_ceiling.get('exhausted')}`, "
            f"selected_rule=`{selected_budget.get('rule')}`, "
            f"scope=`{selected_budget.get('history_match_scope')}`, "
            f"prior/allowed=`{ledger_ratio(selected_budget)}`, "
            f"exhausted=`{selected_budget.get('exhausted')}`"
        ),
        (
            "- primary: "
...
def _l4_kpis_markdown(l4_kpis: dict[str, Any]) -> list[str]:
    ...
    selected_budget = l4_kpis.get("selected_rule_budget") or {}
    return [
        "",
        "## L4 KPI Checklist",
        "",
