...
FAILURE_EPISODE_FIELDS = frozenset(
    {
        "lifecycle_family",
        "status",
        ...
        "line_tolerance",
        "minimum_fault_count",
        "minimum_recovery_attempt_count",
        "minimum_recovery_confirmation_count",
        "maximum_recovery_confirmation_count",
        "required_entities",
...
def validate_gold_label(value: Mapping[str, Any]) -> None:
    ...
    if l0 is not None:
        ...
        _validate_object_list(
            l0.get("required_failure_episodes"),
            FAILURE_EPISODE_FIELDS,
            "gold.l0_expectation.required_failure_episodes",
        )
