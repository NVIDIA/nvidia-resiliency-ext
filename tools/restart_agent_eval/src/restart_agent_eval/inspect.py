...
def _comparison(
    *,
    ...
    model_decision_evidence = model_l0.get("decision_evidence_view")
    ...
    return {
        ...
        "decision_evidence": {
            ...
            "compact_projection_present_in_model_view": isinstance(model_decision_evidence, dict),
            "projection_source_schema_version": (
                model_decision_evidence.get("source_schema_version")
                if isinstance(model_decision_evidence, dict)
                else None
            ),
            "projection_identity_consistent": (
                _decision_evidence_projection_consistent(
                    decision_evidence,
                    model_decision_evidence,
                )
                if isinstance(decision_evidence, dict) and isinstance(model_decision_evidence, dict)
                else False
            ),
        },
        "collection_counts": {
        ...
        "trace_note": (
            "l0_model_view is the exact typed L0B artifact supplied inside the initial "
            "model message. Exact internal Decision Evidence remains a separate trace "
            "artifact; decision_evidence_view is its bounded model-facing projection. "
            "A complete request also contains the system prompt, response schema, "
            "advertised tool schemas, and request options."
        ),
    }


def _decision_evidence_projection_consistent(
    exact: dict[str, Any],
    projected: dict[str, Any],
) -> bool:
    if projected.get("source_schema_version") != exact.get("schema_version"):
        return False
    return all(
        projected.get(field) == exact.get(field)
        for field in (
