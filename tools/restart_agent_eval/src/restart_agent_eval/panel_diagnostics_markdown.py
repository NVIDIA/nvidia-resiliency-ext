...
def _append_l3_history(panel: dict[str, Any], lines: list[str]) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "| target | source | history identity ready | selected root | affected entity |",
            "|---|---|---|---|---|",
        ]
    )
    for row in panel["rows"]:
    ...
    lines.extend(
        [
            "| target | available | same job/root | advanced | no advance | unknown | same failure | entity S/D/U | root/entity streak |",
            "|---|---|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for row in panel["rows"]:
...
def _append_l4_policy(panel: dict[str, Any], lines: list[str]) -> None:
    ...
    lines.extend(
        [
            "| target | decision | basis | retry rule | root ledger | selected ledger | exhausted by | current evidence qualified | quality | NVRx use |",
            "|---|---|---|---|---:|---|---|---|---|---|",
        ]
    )
    for row in panel["rows"]:
    ...
    lines.extend(
        [
            "| target | domain | domain status | retry outlook unchanged | retry status | L2 grounded | observed advance | affected entity | declared capabilities | applied capability |",
            "|---|---|---|---|---|---|---|---|---|---|",
        ]
    )
    for row in panel["rows"]:
        applied_capability = row.get("l4_applied_recovery_capability") or {}
        declared_capabilities = row.get("l4_declared_recovery_capability_ids") or []
        lines.append(
            "| "
            f"{_md(row.get('target'))} | "
            ...
            f"{_md(', '.join(str(item) for item in declared_capabilities))} | "
            f"{_md(applied_capability.get('capability_id'))} |"
        )
...
def _diagnostic_ledger(row: dict[str, Any], prefix: str) -> str:
    ...
    return (
        f"{_md(row.get(f'{prefix}_matching_prior_failures'))}/"
        f"{_md(row.get(f'{prefix}_allowed_retries'))}"
    )


def _diagnostic_selected_ledger(row: dict[str, Any]) -> str:
    if not row.get("l4_selected_rule_budget_present"):
        return "-"
    return (
        f"{_md(row.get('l4_selected_rule_budget_rule'))} "
        f"{_md(row.get('l4_selected_rule_budget_scope'))} "
        f"{_md(row.get('l4_selected_rule_matching_prior_failures'))}/"
        f"{_md(row.get('l4_selected_rule_allowed_retries'))}"
    )
