...
def _append_comparison_axes(panel: dict[str, Any], lines: list[str]) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "| target | first-turn usable | model turns | tool turns | repair turns | tool calls | duplicate | semantic new/no-new/unassessed | final impact | tokens |",
            "|---|---|---:|---:|---:|---:|---:|---|---|---:|",
        ]
    )
    for row in comparison_axes.get("behavioral_efficiency") or []:
        ...
        lines.append(
            ...
            f"{_md(row.get('semantic_new_evidence_calls'))}/"
            f"{_md(row.get('semantic_no_new_evidence_calls'))}/"
            f"{_md(row.get('semantic_novelty_unassessed_calls'))} | "
            f"{_md(row.get('final_context_impact'))} | "
...
def _append_shared_l0(panel: dict[str, Any], lines: list[str]) -> None:
    ...
    lines.append(
        ...
        f"narrative_status=`{shared_l0.get('l0b_narrative_status')}`, "
        f"narrative_events=`{shared_l0.get('l0b_narrative_event_count')}`, "
    ...
    lines.append(f"- section_sizes: `{shared_l0.get('l0b_section_sizes')}`")
...
def _append_l0_coverage_and_tool_profiles(panel: dict[str, Any], lines: list[str]) -> None:
    ...
    for row in panel["rows"]:
        lines.append(
            ...
            f"{_md(row.get('l0_tool_calls_added_lines_beyond_initial_excerpt'))} |"
        )
...
def _append_l1_call_diagnostics(panel: dict[str, Any], lines: list[str]) -> None:
    ...
    lines.extend(
        [
            "| target | tool calls | tool-driven turns | exact new lines | exact no-new calls | duplicate calls | semantic new | semantic no-new | semantic unassessed | complete scans | incomplete scans | sample truncated | tool errors |",
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for row in panel["rows"]:
        lines.append(
            ...
            f"{_md(row.get('l1_kpi_tool_exact_new_line_count'))} | "
            f"{_md(row.get('l1_kpi_tool_exact_no_new_line_calls'))} | "
            ...
            f"{_md(row.get('l1_kpi_tool_complete_source_scan_calls'))} | "
            f"{_md(row.get('l1_kpi_tool_incomplete_source_scan_calls'))} | "
            f"{_md(row.get('l1_kpi_tool_sample_truncated_calls'))} | "
