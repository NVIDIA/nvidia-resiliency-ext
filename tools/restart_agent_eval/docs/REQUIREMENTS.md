...
Rule selection, effective allowed retries, exhaustion, decision basis, and
action MUST be evaluated independently when labeled. Effective allowed retries
means the selected-rule budget when present, the general root ceiling
otherwise, and zero for immediate `workload_unrecoverable`. Review artifacts
...
matching, progress relations, matching prior counts, and root-only versus
root-plus-entity streaks. Unknown progress MUST remain distinct from no
observed advance.
...
Reports MUST expose the selected history-match scope and affected entity.
