...
Quality KPIs SHOULD include required fact/reference selection, deterministic
failure-narrative event/order/reference coverage, narrative semantic restraint,
model-view retention, and fanout-compaction correctness. Operational metrics
MUST include selection/projection latency, per-section view size and estimated
tokens, budget use, selection/compaction counts, narrative/fanout counts,
integrity, and replay consistency.

Model-conditioned diagnostics MUST distinguish:

- required evidence found only through tools: likely L0B coverage gap;
- repeated chronology reconstruction or broad semantically unproductive searches across routes:
  likely L0B presentation gap;
- isolated rereads of already visible evidence: route/model behavior.

Required L0B evidence is visible when its rendered source content appears in
any model-visible L0B section, including context windows, candidate anchors,
and normalized occurrence groups. A provenance reference without rendered
source content does not by itself establish visibility.

The harness MUST use controlled L0B profile ablations before assigning the
second or third category as a product defect or model defect.
...
- Model turns, tool-driven turns, repair turns, and provider retries MUST be
  counted separately.
- Tool reporting MUST keep exact-line novelty, product-reported semantic
  evidence novelty, and final-answer dependency separate. A tool can return a
  new source line without adding semantic evidence, or return a semantically
  useful line that does not appear in the final answer.
- Duplicate reads and calls without exact new lines MUST remain visible as
  diagnostics. They MUST NOT be relabeled as semantic no-new-evidence calls.
- Full-source scan completion and returned-sample truncation MUST be reported
  independently.
...
- Tool-result extraction MUST consume the product trace envelope, including
  line-bearing payloads nested under `result.data`.
