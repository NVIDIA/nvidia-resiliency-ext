...
`eligible_deterministic`, with reason `no_model_enrichment`.
...
unapplied suggestions; L3 root-only/root-plus-entity compatibility and progress
relations; and L4 policy version, retry rule, selected history scope, allowed
retries, matching prior failures,
