...
4. production-compatible terminal latency, plus progressive latency when that
   optional schedule is enabled.
...
deterministic recommendation readiness, route completion, and final completion.
...
Inspect same-job/exact-root attempts, affected-entity relations, typed progress
relations and deltas, unknown comparisons, and root-only versus
root-plus-entity no-observed-advance streaks. L3 does not choose a threshold,
map policy, or emit an action.

### 6. L4 Policy

Compare the raw L1 recovery assessment and L3 history facts with L4's policy
version, selected retry rule and history scope, allowed retries, matching prior failures,
...
Review end-to-end latency, deadline outcome, model-enriched versus deterministic
...
- **Route outcome:** model-enriched versus deterministic use, terminal p50/p90/p99,
  post-`progressive_end` p50/p90/p99, decision-window hit rate, and final NVRx
  usability. Score the shared deterministic recommendation and each available
