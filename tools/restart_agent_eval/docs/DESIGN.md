...
product CLI: collect_all
        ...
        |
        +--> shared L0B narrative + compact view
                            --> parallel independent model routes
                            --> L1 --> L2 --> L3 --> L4
        |
        v
...
evidence indicates a likely L0B coverage gap. Repeated reconstruction of an
already bundled chronology across several routes indicates an L0B presentation
gap. An isolated route rereading bundled evidence remains model/prompt/profile
behavior. The harness assigns ownership through controlled L0B profile
ablations rather than one run's payload size or tool count. L2 is grounding,
identity, and advisory audit; it does not rewrite L1 semantics. L3 reports
root-only and root-plus-entity recurrence without selecting a threshold, and L4
owns policy/action and match scope.
