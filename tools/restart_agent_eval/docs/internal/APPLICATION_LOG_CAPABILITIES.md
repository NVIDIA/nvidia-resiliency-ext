...
> capability framing and earlier policy exploration for evaluation and
...
| Failure domain | Restart recovery outlook | Evidence or capability | Retry rule | Initial action | Allowed retries |
| --- | --- | --- | --- | --- | ---: |
...
| Workload | May recover or unknown | Grounded managed mechanism plus enabled workload recovery capability | `workload_managed_recovery` | `RESTART` | Capability-defined; bad-token MVP uses 2 |
...
Managed recovery is policy outside the model. L1 may identify a mechanism such
as numerical instability caused by a bad token. L2 grounds that mechanism. A
deterministic policy classifier combines the grounded canonical mechanism with
an enabled workload capability, such as retry-then-skip, and selects
`workload_managed_recovery`. Model prose alone cannot activate the rule.
...
| Bad-token numerical instability with `bad_token_retry_then_skip` enabled and an exact token/sample/window identity | Two-retry `workload_managed_recovery` budget counted only for the same root and entity. |
