...
The product exposes two typed history identities: a `root_fingerprint` for the
failure family/mechanism and an optional `affected_entity` for an exact artifact
or data position. L0 owns the deterministic identity; L2 owns each grounded enriched
identity. The harness measures cross-route agreement and corpus false
merge/split behavior for both.
