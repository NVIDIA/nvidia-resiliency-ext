...
| Decision Evidence shared artifact | Canonical L0A facts required by downstream decision logic | Compact deterministic identity, failure, progress, recovery, and placement facts | Deterministic L3/L4 and model-visible L0B projection |
...
| L2 Grounding, Identity, And Audit | Raw L1 output, L0A bundle, L0B view, tool evidence, source log | Minimally grounded `AttemptFailureFacts` with root and optional affected entity, mechanical reference repairs, and separate audit findings/unapplied semantic suggestions | L3 current facts and observability; L4 receives unchanged L1 semantics |
| L3 History Enrichment | Client-derived root/entity identity, current progress, job/cycle identity, immutable runtime-selected prior attempts | Ordered exact-root comparisons, affected-entity relations, typed progress relations/deltas, and root-only/root-plus-entity streaks | L4 |
| L4 Policy Decision | Unchanged L1 assessment or deterministic facts, observational L3 history, policy profile | Selected retry rule and history scope, allowed/count/exhausted state, decision basis, action, and provenance | Candidate selector/NVRx |
...
L2 owns this enriched-path fingerprint. L0 owns the deterministic-path fingerprint.
...
- selected-path `AttemptFailureFacts` from L2 grounding or the deterministic L0 deterministic;
...
same/different/unknown affected-entity relation per root match
...
exact failure-iteration matches
consecutive same-root no-observed-advance count
consecutive same-root-and-entity no-observed-advance count
...
same grounded root fingerprint. It additionally compares typed affected
...
current MVP L4 rule chooses the root-only or root-plus-entity advancement/streak
declared by its policy rule; the absolute summary remains separately visible.
...
selects a versioned retry rule and history-match scope, records the allowed-retry budget and matching
...
| `confirmation_retry` | Exact replay-stable affected entity from deterministic or grounded evidence | configurable, default `1`, exact root and entity |
...
| `workload_managed_recovery` | Declared bad-token retry/skip capability plus grounded data-position entity | configured `2`; exact root plus entity |

Only distinct prior cycles matching the selected rule's root-only or
root-plus-entity scope and showing no observed advance count toward exhaustion.
Unknown progress does not count, and observed advance in that scope prevents
exhaustion for the current decision.

### Functional Outputs: `L4DecisionOutput` And `AnalysisResult`

```text
retry_policy
  policy_version, selected rule, retry_budget_exhausted, exhausted_by
  general_root_ceiling:
    root-only allowed retries, matching prior failures, observed advance,
    exhaustion, applicability
  selected_rule_budget (optional):
    selected rule, root-only or root-and-entity scope, allowed retries,
    matching prior failures, observed advance, exhaustion
