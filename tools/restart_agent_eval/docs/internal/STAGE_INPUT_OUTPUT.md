...
facts needed by history and policy. L0B orders those facts into a deterministic
failure narrative, projects Decision Evidence for model consumption, and adds
selected supporting evidence from L0A. The result is the bounded,
attention-efficient initial view presented to the model.
...
| L0B Initial Model Evidence View | Exact Decision Evidence, L0A support, and L0B evidence-selection rules and size limits | Deterministic failure narrative, compact Decision Evidence view, and bounded supporting evidence | L1 initial request |
...
| L0B | Narrative templates/order, Decision Evidence compaction, supporting-evidence selection, size limits, deduplication, and lossiness |
...
L0B: deterministic narrative + compact decision evidence + selected L0A support
...
is its canonical policy-relevant selection, and L0B is a deterministic narrative
plus compact facts and bounded supporting detail. The exact contracts remain in
the product evidence and schema specs.
...
L0B selects and serializes the evidence initially presented to L1. It first
constructs an ordered failure narrative from typed current-log facts, then adds
a compact Decision Evidence projection and bounded original support selected
from L0A. It reduces attention spent on routine progress, repeated rank output,
duplicate stack renderings, and downstream teardown.

The narrative uses fixed event kinds and templates. Applicable events cover
prior operation success, last progress/checkpoint, current operation start,
fault observation and recovery, primary or selected observation, fanout/cascade,
and later-progress outcome. Every event has source references. Known unknowns
state only mechanically established evidence boundaries; they do not infer a
semantic cause, failure domain, retry outlook, history conclusion, or action.

Large fanout is represented by an exact count and bounded rank/line samples.
The complete member set remains in L0A and trace rather than consuming initial
model attention.
...
```text
ordered deterministic failure narrative with source references
compact policy-relevant Decision Evidence view
compact run/job/progress context
selected failure episodes and distributed incidents as supporting detail
...
- deterministic narrative event, order, and reference coverage;
- narrative semantic restraint: no model-like cause, domain, recovery, history,
  or action inference;
- relevant progress/checkpoint/supporting-context coverage;
- compaction safety: exact fanout counts and required evidence survive bounded
  sampling; and
- first-turn completion, tool-only required context, bundled-evidence reread,
  and broad no-new exploration across models and profile ablations.

#### L0B Operational Metrics

- projection latency;
- total and per-section model-view characters and estimated tokens;
...
rank-equivalent exception copies. L0B first renders prior progress, checkpoint
load start, terminal decode failure, compact fanout, teardown, and no later
progress as an ordered narrative. It then selects five windows, compacts the
copies to one representative plus a count, and preserves lines 11800, 12000,
12083, and 12135. An illustrative score is:

```text
required evidence coverage:         4/4 gold lines/facts represented
primary retention:                  pass (12083 remains visible)
narrative coverage:                 6/6 typed events in expected order
narrative semantic restraint:       pass (ambiguity preserved)
...
no-new narrative reconstruction:    0 calls
...
- default advertised read-only tools: `grep_log`, `read_window`, and bounded
  `get_evidence_objects`; `overview` is implemented but advertised only when
  its profile boolean is enabled;
