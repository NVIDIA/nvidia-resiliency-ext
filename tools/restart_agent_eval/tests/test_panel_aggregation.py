...
class PanelAggregationTest(unittest.TestCase):
    ...
    def test_panel_preserves_three_tracks_and_selected_path(self) -> None:
        summary = {
            "target": "gpt",
            "model": "test-gpt",
            "failure_tracks": {
            ...
            "l3_history": {
                ...
                "routes": {
                    "gpt": {
                        ...
                        "observation": {
                            "available": False,
                            "availability_reason": "no_matching_observation",
                        },
                    }
                },
            },
        ...
        panel = _panel_summary([summary])
        row = panel["rows"][0]

        self.assertEqual(row["primary_track_status"], "grounded")
        self.assertEqual(row["observation_track_status"], "grounded")
        self.assertEqual(row["primary_history_status"], "available")
        self.assertEqual(row["observation_history_status"], "no_matching_observation")
        self.assertEqual(row["l4_selected_path"], "primary")
        self.assertEqual(row["l4_path_selection_reason"], "grounded_primary_available")
    ...
    def test_panel_reports_fingerprint_stability_and_disagreement_reason(self) -> None:
        ...
        self.assertTrue(panel["affected_entity_agreement"]["all_available_agree"])

        different_primary = json.loads(json.dumps(second))
        different_primary["target"] = "claude"
        different_primary["primary_selection_by_stage"]["l2_grounded"]["line"] = 12084
        different_primary["l2_kpis"]["stable_identity_anchor_line"] = 12084
        different_primary["l2_kpis"]["affected_entity"][
            "fingerprint"
        ] = "affected_entity:artifact:different"
        split_panel = _panel_summary([summary, different_primary])
        self.assertEqual(
            split_panel["affected_entity_agreement"]["disagreement_reason"],
    ...
    def test_observation_only_routes_do_not_report_missing_root_fingerprints(self) -> None:
        summaries = []
        for target in ("gpt", "gemini"):
            summaries.append(
                {
                    "target": target,
                    "model": target,
        ...
        panel = _panel_summary(summaries)
        categories = {item["category"] for item in panel["concerns"]}

        self.assertEqual(panel["l2_root_fingerprint_agreement"]["available_models"], 0)
        self.assertEqual(panel["l2_observation_fingerprint_agreement"]["status"], "stable")
        self.assertNotIn("l2_root_fingerprint_stability", categories)
        self.assertEqual(panel["l2_identity_kind_counts"], {"observation_only": 2})
        self.assertEqual(panel["history_identity_kind_counts"], {"observation_only": 2})

    def test_l2_unavailability_does_not_imply_final_identity_disagreement(self) -> None:
        summaries = []
        for target, l2_identity_kind in (("gpt", "observation_only"), ("qwen", "none")):
            summaries.append(
                {
                    "target": target,
                    "model": target,
                    "decision": "RESTART",
                    "l2_kpis": {"identity_kind": l2_identity_kind},
                    "history_identity_comparison": {
                        "identity_kind": "observation_only",
                        "history_identity_ready": True,
                        "observation_fingerprint": (
                            "terminal_transport_failure_surface:tcpstore_connection_loss"
                        ),
                        "source": "l2_grounded" if target == "gpt" else "l0_deterministic",
                    },
                }
            )
        ...
        self.assertEqual(
            panel["l2_identity_kind_counts"],
            {"none": 1, "observation_only": 1},
        )
        self.assertEqual(
            panel["history_identity_kind_counts"],
            {"observation_only": 2},
        )
        self.assertNotIn("history_identity_kind_disagreement", categories)
    ...
    def test_comparison_axes_separate_model_endpoint_and_route_outcomes(self) -> None:
            ...
            [
                {
                    ...
                    "l4_kpis": {
                        ...
                        "model_contribution": "attempted_not_used_timeout",
                    },
                },
                {
                    "target": "usable-route",
                    ...
                    "l4_kpis": {
                        ...
                        "nvrx_use": "eligible",
                        "model_contribution": "attempted_used",
                    },
                },
            ]
        )
        ...
        self.assertEqual(axes["route_outcome"][0]["nvrx_use"], "eligible_deterministic")
        self.assertEqual(
            axes["route_outcome"][0]["reason"],
            "attempted_not_used_timeout",
        )
        self.assertEqual(
            axes["route_outcome"][1]["model_contribution"],
            "model_enriched",
        )
        self.assertEqual(axes["route_outcome"][1]["reason"], "attempted_used")

    def test_usable_but_ungrounded_model_output_is_deterministic_only(self) -> None:
        panel = _panel_summary(
            [
                {
                    "target": "qwen397b",
                    ...
                    "l4_kpis": {
                        "model_contribution": "attempted_not_used_ungrounded",
                        "result_quality": "degraded",
                        "nvrx_use": "eligible_degraded",
                    },
                }
        ...
        outcome = panel["comparison_axes"]["route_outcome"][0]
        self.assertEqual(outcome["model_contribution"], "deterministic_only")
        self.assertEqual(outcome["reason"], "attempted_not_used_ungrounded")
        self.assertEqual(outcome["nvrx_use"], "eligible_deterministic")

    def test_panel_excludes_deterministic_from_model_scoring(self) -> None:
        deterministic = {
            "target": "deterministic",
            "model": None,
            "decision": "RESTART",
