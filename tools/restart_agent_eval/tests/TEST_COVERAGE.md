...
| `panel.py` | `test_panel_aggregation.py`, `test_panel_cli.py`, `test_panel_rendering.py` | Empty/missing run, malformed reviews, route disagreement, missing routes, endpoint/tool concerns, deterministic/enriched policy, shared L0 consistency. |
...
| `review_context.py` | `test_review_context.py` | Complete and sparse traces, selected deterministic/enriched analysis, malformed optional fields, injected artifact store. |
