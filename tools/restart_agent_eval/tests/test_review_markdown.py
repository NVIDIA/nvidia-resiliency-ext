...
class ReviewMarkdownTest(unittest.TestCase):
    def test_review_markdown_publishes_core_route_outcome(self) -> None:
            ...
            "retry_policy": {
                **retry_policy(
                ...
                "current_affected_entity": {
                    "kind": "data_position",
                    "identity": "token:42",
                    "fingerprint": "affected_entity:data_position:token-42",
                },
            },
        ...
        self.assertIn("applied_capability=`bad_token_retry_then_skip`", rendered)
        self.assertIn("scope=`root_and_entity`", rendered)
        self.assertIn("affected_entity=`data_position:token:42`", rendered)
