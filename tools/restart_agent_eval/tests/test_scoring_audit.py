...
class AuditScoringTest(unittest.TestCase):
    ...
    def test_retry_policy_signal_is_reported_without_rewriting_l1(self) -> None:
        ...
        self.assertEqual(signals["retry_policy_base_rule"], "general_retry")
    ...
    def test_l2_kpis_count_observational_findings_without_policy_weight(self) -> None:
        ...
        self.assertEqual(kpis["observational_finding_count"], 2)
        self.assertEqual(
            kpis["finding_severity_counts"],
            {"advisory": 1, "credibility": 1},
        )
        summaries = [
            {
                "target": "qwen235b",
    ...
    def test_dangling_evidence_reference_remains_observational(self) -> None:
        kpis = scoring.l2_kpis(
            l2_audit={
                "used": True,
                "audit_status": "findings",
                "grounding_status": "grounded",
                "selected_observation_used": True,
                "field_findings": {
                    "observed_failures.evidence_ids": [
                        "selected observation references undefined evidence ids: line-30368"
                    ]
                },
                "findings": [
                    {
        ...
        self.assertEqual(kpis["grounding_status"], "grounded")
        self.assertTrue(kpis["selected_observation_available"])
        self.assertEqual(kpis["finding_count"], 1)
        self.assertEqual(kpis["observational_finding_count"], 1)
        self.assertEqual(kpis["finding_severity_counts"], {"credibility": 1})
