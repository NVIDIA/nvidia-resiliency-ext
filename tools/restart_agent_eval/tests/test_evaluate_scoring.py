...
class EvaluationScoringTest(unittest.TestCase):
    ...
    def test_score_checks_recovery_rule_decision_evidence_and_primary(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = eval_harness.score_case(
                run_id="run-a", target="gpt", case=case, verdict=verdict
            )
    ...
    def test_deterministic_target_marks_enrichment_stages_not_applicable(self) -> None:
        result = eval_harness.score_case(
            run_id="run-a",
            target="deterministic",
            case=_case(
                label={"l0b_expectation": {"required_evidence_lines": [12]}},
        ...
        aggregate = eval_harness.aggregate_results("run-a", "deterministic", [result])

        self.assertIsNone(result.l0b_quality_correct)
        self.assertIsNone(result.l1_recovery_correct)
        self.assertEqual(result.l1_recovery_fields, {})
        self.assertIsNone(result.l2_audit_correct)
        self.assertIsNone(aggregate["l0b_quality_accuracy"])
        self.assertIsNone(aggregate["l1_recovery_accuracy"])

    def test_deterministic_primary_line_counts_as_result_evidence(self) -> None:
        result = eval_harness.score_case(
            run_id="run-a",
            target="deterministic",
            case=_case(label={"l0b_expectation": {"required_evidence_lines": [12]}}),
            verdict={
                "decision": "RESTART",
                "primary_failure": {"line": 12},
            },
    ...
    def test_corpus_score_separates_deterministic_from_enriched_decision(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            log_root, gold_root = write_evaluation_case(root)
            case = eval_harness.discover_cases(log_root, gold_root)[0]
        ...
        self.assertFalse(result.deterministic_decision_correct)
        ...
        self.assertEqual(aggregate["deterministic_decision_accuracy"], 0.0)
