...
class ReviewRoutePlanningTest(unittest.TestCase):
    ...
    def test_deterministic_target_explicitly_disables_product_enrichment(self) -> None:
        target = expand_targets(["deterministic"])[0]
        args = review_log.parse_review_args(["--log", "/tmp/example.log", "deterministic"])
        executor = _RecordingExecutor(process_result(stdout="{}"))
        ...
        command, _, _ = executor.calls[0]
        self.assertIn("--disable-l1", command)
        self.assertNotIn("--enable-l1", command)
