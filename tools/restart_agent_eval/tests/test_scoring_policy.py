...
"""Gold scoring for deterministic and L1-enriched L4 policy."""
...
class PolicyGoldScoringTest(unittest.TestCase):
    def test_gold_scores_deterministic_and_enriched_policy_paths_separately(self) -> None:
        gold = {
            "case_id": "code-error",
            "label_version": 1,
        ...
        self.assertFalse(score["deterministic_l4"]["action_correct"])
        self.assertFalse(score["deterministic_l4"]["policy_action_pass"])
    ...
    def test_gold_scores_base_and_policy_context_rules_independently(self) -> None:
        gold = {
            "retry_policy_expectation": {
                "accepted_rules": ["general_retry"],
        ...
        view = {
            ...
            "retry_policy": {
                ...
                "effective_policy": {
                    ...
                    "rule": "rejected_iteration_retry_then_skip",
                    "allowed_retries": 2,
                },
            },
        }
        ...
        self.assertTrue(score["retry_rule_correct"])
        self.assertTrue(score["policy_action_pass"])

        view["retry_policy"]["effective_policy"]["rule"] = "general_retry"
        mismatch = scoring.score_semantic_view(view, gold, include_action=True)
        self.assertFalse(mismatch["retry_rule_correct"])
