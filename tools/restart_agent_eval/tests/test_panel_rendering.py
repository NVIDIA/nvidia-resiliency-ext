...
class PanelRenderingTest(unittest.TestCase):
    def test_panel_markdown_separates_summary_from_diagnostics(self) -> None:
        ...
        self.assertIn("## Deterministic Path Inputs", diagnostics_markdown)
        self.assertIn("## L3 History", diagnostics_markdown)
        self.assertIn("## L4 Policy Decision", diagnostics_markdown)
        self.assertIn("| root ledger | selected ledger | job guards |", summary_markdown)
        self.assertIn("concrete_confirmation_retry:0/1", summary_markdown)
        self.assertIn("concrete_confirmation_retry root_and_entity 0/1", diagnostics_markdown)
    ...
    def test_unknown_recovery_confidence_is_not_presented_as_a_score(self) -> None:
        summary = rich_route_summary()
        summary["model_recovery_assessment"] = recovery_assessment(
        ...
        panel, markdown, diagnostics = published_panel([summary])

        row = panel["rows"][0]
        self.assertEqual(row["model_failure_domain_confidence"], 87)
        self.assertEqual(row["model_retry_outlook_confidence"], 92)
        self.assertIn("| unknown | unknown | N/A | unknown | unknown | N/A |", markdown)
        self.assertIn("| unknown | unknown | N/A |", diagnostics)

    def test_panel_reports_grounded_policy_inputs_and_affected_entity(self) -> None:
        summary = rich_route_summary()
        summary["l4_kpis"] = {
            ...
            "current_affected_entity": summary["current_failure_facts"]["affected_entity"],
        }

        _, _, diagnostics = published_panel([summary])

        self.assertIn("supported_but_unconfirmed", diagnostics)
        self.assertIn("root_and_entity", diagnostics)
        self.assertIn("artifact:/lustre/checkpoints/iter_5000/metadata", diagnostics)

    def test_panel_reports_affected_entity_stability_across_models(self) -> None:
        first = rich_route_summary()
        second = rich_route_summary()
        second["target"] = "gpt"
        second["model"] = "test-gpt"

        panel, _, diagnostics = published_panel([first, second])

        agreement = panel["affected_entity_agreement"]
        self.assertEqual(agreement["status"], "stable")
        self.assertEqual(agreement["available_models"], 2)
        self.assertEqual(agreement["unique_fingerprints"], 1)
        self.assertIn("affected-entity agreement", diagnostics)
    ...
    def test_panel_reports_deterministic_and_enriched_policy_scores_separately(self) -> None:
        summary = {
            "target": "gpt",
            "model": "test-gpt",
            "decision": "STOP",
        ...
        self.assertEqual(comparison["deterministic_consistency"], "consistent")
        self.assertEqual(comparison["shared_deterministic"]["decision"], "RESTART")
    ...
    def test_panel_reports_shared_distributed_incident_without_member_fanout(
        ...
        summary = {
            "target": "qwen235b",
            "decision": "RESTART",
            "retry_policy": retry_policy(base_rule="general_retry", allowed_retries=2),
            "primary_failure": {
