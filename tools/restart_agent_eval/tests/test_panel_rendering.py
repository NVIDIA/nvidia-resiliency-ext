...
class PanelRenderingTest(unittest.TestCase):
    def test_panel_markdown_separates_summary_from_diagnostics(self) -> None:
        ...
        self.assertIn("| root ledger | selected ledger | exhausted by |", summary_markdown)
        self.assertIn("bounded_retry:0/1", summary_markdown)
        self.assertIn("bounded_retry root_only 0/1", diagnostics_markdown)
    ...
    def test_panel_reports_declared_and_applied_recovery_capability(self) -> None:
        summary = rich_route_summary()
        summary["l4_kpis"] = {
            "declared_recovery_capability_ids": ["bad_token_retry_then_skip"],
            "applied_recovery_capability": {"capability_id": "bad_token_retry_then_skip"},
        ...
        self.assertIn("declared capabilities", diagnostics)
        self.assertIn("applied capability", diagnostics)
        self.assertIn("bad_token_retry_then_skip", diagnostics)
    ...
    def test_panel_reports_shared_distributed_incident_without_member_fanout(
        ...
        summary = {
            "target": "qwen235b",
            "decision": "RESTART",
            "retry_policy": retry_policy(rule="general_retry", allowed_retries=3),
            "primary_failure": {
