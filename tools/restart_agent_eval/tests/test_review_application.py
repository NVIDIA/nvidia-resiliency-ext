...
class _FailedBeforeArtifactsExecutor:
    def run(self, command, *, cwd, env=None):
        return process_result(
            command,
            returncode=7,
...
class ReviewApplicationTest(unittest.TestCase):
    ...
    def test_product_failure_without_artifacts_preserves_original_error(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            log_path = root / "input.log"
            log_path.write_text("failure\n", encoding="utf-8")
            ...
            with redirect_stderr(stderr):
                exit_code = review.ReviewApplication(
                    environment={},
                    process_executor=_FailedBeforeArtifactsExecutor(),
                    clock=_Clock(),
                ).run(args)

            review_files = list(run_dir.glob("*.review.json"))

        self.assertEqual(exit_code, 7)
        self.assertIn("ModuleNotFoundError: No module named 'httpx'", stderr.getvalue())
        self.assertNotIn("unsupported restart-agent trace schema", stderr.getvalue())
