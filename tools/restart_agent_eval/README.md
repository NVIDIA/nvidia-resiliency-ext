...
not contracts. This includes
[`APPLICATION_LOG_CAPABILITIES.md`](docs/internal/APPLICATION_LOG_CAPABILITIES.md),
which preserves broader product framing and earlier policy exploration without
competing with the canonical product specifications.

## Architecture

```text
source log + optional human gold
  -> product collect_all (explicit enrichment profile)
       -> shared L0A and deterministic recommendation
       -> shared L0B and parallel model routes
...
[`configs/model_panel.json`](configs/model_panel.json) retains the multi-route
product configuration used for direct panel experiments. It is evaluation
configuration, not a public product example.
...
uses product `collect_all` with `enrichment.enabled=true`: L0A is built once,
the deterministic recommendation is retained, and one L0B view is replayed
identically to every model route. Shared L0 artifacts appear when L0 completes;
each route result, trace, and review appears when that route completes; panel
artifacts appear after the batch closes. This explicit harness profile does not
change the product default, which has enrichment disabled.
...
route. The route review contains compact semantic, KPI, and gold summaries and
points to the adjacent `result.json` for complete stage output. Within the
result, `l1_assessment` is the unchanged parsed model answer and `l2_grounding`
is the separate client grounding/audit projection. `trace.json` is reserved for
raw responses, tool interaction, and full stage diagnostics.
...
Behavior fixtures characterize product L0, DecisionEvidence, L0B, deterministic
