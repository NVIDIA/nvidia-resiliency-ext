...
def test_scheduler_exclusion_dir_is_passed_to_rendezvous(tmp_path):
    from nvidia_resiliency_ext.fault_tolerance.launcher import config_from_args, get_args_parser

    parser = get_args_parser()
    args = parser.parse_args(["--ft-scheduler-exclusion-dir", str(tmp_path), "train.py"])
    ...
    assert config.fault_tol_cfg.scheduler_exclusion_dir == str(tmp_path)
    assert config.rdzv_configs["scheduler_exclusion_dir"] == str(tmp_path)


def test_scheduler_exclusion_dir_must_be_absolute():
    from nvidia_resiliency_ext.fault_tolerance.launcher import config_from_args, get_args_parser

    parser = get_args_parser()
    args = parser.parse_args(["--ft-scheduler-exclusion-dir", "relative/path", "train.py"])
