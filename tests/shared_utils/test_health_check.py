...
class TestAttributionService(unittest.TestCase):
    ...
    def test_get_last_result_skips_after_decision_budget_expires(self):
        service = AttributionService(endpoint="http://attr.example:8000/", decision_timeout=7.0)
        service._last_submitted = "/tmp/train.log"
        service._terminal_deadline = 20.0

        with (
            patch.object(service, "_get_results") as mock_get,
            patch(
                "nvidia_resiliency_ext.shared_utils.health_check.time.monotonic",
                return_value=21.0,
            ),
    ...
    def test_get_last_result_uses_short_timeout_for_remaining_budget(self):
        service = AttributionService(endpoint="http://attr.example:8000/", decision_timeout=7.0)
        service._last_submitted = "/tmp/train.log"
        service._terminal_deadline = 20.0
        ...
        self.assertEqual(mock_get.call_args.args, ("/tmp/train.log",))
        self.assertAlmostEqual(mock_get.call_args.kwargs["timeout"], 0.2)
