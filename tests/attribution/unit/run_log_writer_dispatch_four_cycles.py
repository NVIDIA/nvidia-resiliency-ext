"""Script 1/2: write four 5-minute per-cycle log files for the dispatch test.
...
Total wall time for the default 4 × 5 = 20 minutes. All four files end
...
def _parse_checkpoint_cycles(value: str, num_cycles: int) -> set[int]:
    """Resolve the ``--checkpoint-cycles`` CLI value to a set of indices."""
...
def main() -> int:
    ...
    print(
        ...
        f"checkpoint cycles: {ckpt_summary}",
        flush=True,
    )
        ...
        for cycle, path in enumerate(paths):
            ...
            for chunk_idx, tmpl in enumerate(CHUNK_TEMPLATES):
                ...
                with open(path, "a", encoding="utf-8") as f:
                    f.write(content)
                suffix = " + checkpoint" if ckpt_emitted else ""
                print(
