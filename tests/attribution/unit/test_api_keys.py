    ...
    def test_raises_when_api_key_empty(self):
        async def run():
            with self.assertRaises(ValueError) as ctx:
                await merge_log_fr_llm("log", "fr", nvidia_api_key="", model="dummy-model")
            self.assertIn("NVIDIA API key", str(ctx.exception))
