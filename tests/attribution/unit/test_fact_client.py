...
def test_fact_attributor_node_list_preserves_plain_hostname_suffixes():
    nodes = fact_client._fact_attributor_node_list(
        ["rack_a_node_1", "rack_a_node_2", "rack_a_node_1"]
    )

    assert nodes == ["rack_a_node_1", "rack_a_node_2"]
...
def test_raw_loki_timestamp_anchor_stays_near_collection_end():
    start_time = datetime(2026, 5, 10, 12, 0, 0, tzinfo=timezone.utc)
    end_time = start_time + timedelta(minutes=12)

    anchor_ns = fact_client._raw_loki_timestamp_anchor_ns(start_time, end_time)

    assert anchor_ns == int((end_time - timedelta(seconds=1)).timestamp() * 1_000_000_000)


def test_raw_loki_timestamp_anchor_stays_inside_short_interval():
    start_time = datetime(2026, 5, 10, 12, 0, 0, tzinfo=timezone.utc)
    end_time = start_time + timedelta(milliseconds=100)

    anchor_ns = fact_client._raw_loki_timestamp_anchor_ns(start_time, end_time)

    assert int(start_time.timestamp() * 1_000_000_000) < anchor_ns
    assert anchor_ns < int(end_time.timestamp() * 1_000_000_000)
...
def test_attributor_info_uses_slurm_job_name(monkeypatch):
    monkeypatch.setenv("SLURM_JOB_NAME", "resnet-training")
    service = fact_client.FactAttributionService(url="http://fact.example/latest")
    timestamp = datetime(2026, 5, 10, 12, 0, 0, tzinfo=timezone.utc)

    info = service._build_attributor_info(
        job_id="job-1",
        cycle_index=2,
        nodes=["node-a"],
        ranks_per_node=4,
    ...
    assert info["workload"]["name"] == "resnet-training"
    assert info["workload"]["username"] == "slurm-user"
    assert info["metadata"]["cluster"] == "slurm-cluster"


def test_attributor_info_defaults_job_name_when_slurm_job_name_missing(monkeypatch):
    ...
    assert info["workload"]["name"] == "unknown"
