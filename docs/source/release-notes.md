...
- Node health check service integration
    ...
    - Example: `--ft-node-health-check-endpoint unix:///var/run/nvhcd.sock`. If the endpoint is not configured, or the optional service is unavailable, NVRx skips the external node health check. NVRx marks the node unhealthy only when the service returns structured JSON with `fail_count > 0`; unusable signals are logged and ignored.
