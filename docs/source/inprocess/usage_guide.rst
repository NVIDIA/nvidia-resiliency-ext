...
`NCCL <https://docs.nvidia.com/deeplearning/nccl/release-notes/rel_2-26-2.html/>`_ v2.26.2 or higher, 
`PyPI PyTorch <https://pypi.org/project/torch/>`_ v2.5.1 through v2.6.0 or `PyTorch NGC
...
- currently if there are multiple NCCL communicators per process group, PyTorch aborts the communicators 
  sequentially. To avoid deadlocks, it's required that all distributed groups for P2P communicators need 
  to be created and initialized explicitly at python level or alternatively if the workload uses P2P 
  communication without creating explicit 2-rank groups, then please refer to this 
  `PR <https://github.com/pytorch/pytorch/pull/150690>`_ for the deadlock issue details and make sure 
  you've an updated version of PyTorch with the fix supporting NCCL group abort.  
